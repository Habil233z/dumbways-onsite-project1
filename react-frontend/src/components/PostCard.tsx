...
export default function PostCard() {
    ...
    return (
        ...
        <div className="overflow-y-scroll w-[90%] bg-gray-100 flex flex-col items-center rounded-4xl pb-20">
        {post.map((item) => {
            return (
                <div className="w-full items-center justify-center" key={item.id}>
                <Link to={`/postDetail/${item.id}`}>
                <div className="w-100% min-h-40 bg-white flex border border-gray-900 rounded-4xl m-5 shadow-xl">
                    <div className="flex">
                    ...
                    <div  className="w-full ml-5">
                        ...
                        <div className="flex mt-8 flex-row-reverse">
                            ...
                            <Button className="w-20 mr-5" id={"reply"+ item.id} onClick={(e) => {e.stopPropagation(); handleReply(e, item.id)}}>Reply</Button>
                        </div>
                    </div>
                </div>
                </Link>
