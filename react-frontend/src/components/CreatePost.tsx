...
export default function CreatePost() {
    ...
    return (
        <div className="fixed h-20 w-full bg-gray-200 border-t-2 mt-500 inset-x-0 bottom-0 dark:bg-gray-900 dark:border-gray-700" id="createPost">
            ...
            <Button className="bg-gray-950 mt-5 dark:bg-gray-500 dark:hover:bg-gray-600 dark:active:bg-gray-700" onClick={() => setCreateForm(true)}>Create Post</Button>
            </div>
            {createForm && <div className="fixed inset-0 h-full w-ful bg-gray-950 opacity-50" onClick={() => setCreateForm(false)}></div>}
            {createForm && 
                <div className="fixed h-100 w-[50%] bg-gray-200 top-50  ml-[25%] mr-auto flex flex-col justify-center items-center border-4 border-gray-900 shadow-2xl dark:bg-gray-800">
                    ...
                    <input type="file" onChange={(e: any) => setSelectedFile(e.target.files[0])} accept="image/*" className="bg-gray-400 border-4 border-gray-900 mt-5 w-60 pl-5"/>
                    <div className="flex items-center justify-center">
